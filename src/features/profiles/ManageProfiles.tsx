...
const ManageProfiles = () => {
  ...
  const { data: profiles, isLoading } = useQuery({
    queryKey: ["profiles"],
    queryFn: async () => {
      const response = await getAPI().profiles.getAllCategories({ format: "json" });
      return Array.isArray(response) ? response : response.data;
    },
  });

  const paginatedProfiles = profiles?.slice((page - 1) * rowsPerPage, page * rowsPerPage);
  ...
  return (
    ...
    <Card>
              ...
              <TableBody>
                {paginatedProfiles?.map((profile: ProfileObject) => (
                  <TableRow key={profile.id}>
                    ...
                    <TableCell>
                      <IconButton color="primary" onClick={() => setViewDetails(profile)}>
                        <Visibility />
                      </IconButton>
                      <IconButton color="warning" onClick={() => handleEdit(profile)}>
                        <Edit />
                      </IconButton>
                      <IconButton
                        color="error"
      ...
      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>{editId ? "Editar Perfil" : "Crear Perfil"}</DialogTitle>
        <DialogContent>
          <TextField
            margin="normal"
            fullWidth
      ...
      <Dialog open={!!viewDetails} onClose={() => setViewDetails(null)} fullWidth maxWidth="sm">
        <DialogTitle>Detalles del Perfil</DialogTitle>
        <DialogContent dividers>
          {viewDetails && (
            <>
              <Typography variant="h6" gutterBottom>Nombre: {viewDetails.name}</Typography>
              <Typography variant="body1">
                Fechas: {viewDetails.startDate?.substring(0, 10)} - {viewDetails.endDate?.substring(0, 10)}
              </Typography>
              <Typography variant="body1">Activo: {viewDetails.active ? "Sí" : "No"}</Typography>
              <Typography variant="body1">Externo: {viewDetails.external ? "Sí" : "No"}</Typography>
              ...
              {viewDetails.skills?.length ? (
                ...
                <Typography variant="body2" color="text.secondary">Sin habilidades asociadas</Typography>
              )}

              <Divider sx={{ my: 2 }} />

              <Typography variant="h6" gutterBottom>Recurso</Typography>
              {viewDetails.resource ? (
                <>
                  <Typography variant="body1">Nombre: {viewDetails.resource.firstName} {viewDetails.resource.lastName}</Typography>
                  <Typography variant="body1">Puesto: {viewDetails.resource.jobTitle}</Typography>
                  <Typography variant="body1">Experiencia: {viewDetails.resource.experience}</Typography>
                  <Typography variant="body1">Ubicación: {viewDetails.resource.location}</Typography>
                  <Typography variant="body1">Usuario: {viewDetails.resource.user}</Typography>
                </>
              ) : (
                <Typography variant="body2" color="text.secondary">Sin recurso asignado</Typography>
              )}
            </>
          )}
        </DialogContent>
